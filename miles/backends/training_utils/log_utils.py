...
def _reduce_gathered_log_dicts(
    ...
    `reduction_by_key` ("mean", "min" or "max"); keys not listed reduce by mean.
...
def gather_log_data(
    ...
    `reduction_by_key` names "min" or "max" for them. The DP source rank prints
    and optionally logs to WandB/TensorBoard with a step derived from
    `rollout_id` and batch sizes. Returns the reduced dict on the DP source
...
def log_multi_turn_data(rollout_id: int, args: Namespace, rollout_data: RolloutBatch) -> None:
    ...
    if parallel_state.tp.rank == 0 and parallel_state.is_pp_last_stage:
        ...
        reduction_by_key = {
            key: "max" if key.endswith("_max") else "min" for key in log_dict if key.endswith(("_max", "_min"))
        }
        gather_log_data("multi_turn", args, rollout_id, log_dict, reduction_by_key=reduction_by_key)
