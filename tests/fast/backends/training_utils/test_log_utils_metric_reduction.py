...
def test_min_max_keys_reduce_to_global_extrema():
    ...
    reduced = log_utils._reduce_gathered_log_dicts("multi_turn", gathered, reduction_by_key)
...
def test_keys_default_to_mean_without_reduction_map():
    gathered = [
        {"multi_turn_metric/round_number_mean": 2.0},
        {"multi_turn_metric/round_number_mean": 4.0},
    ]

    reduced = log_utils._reduce_gathered_log_dicts("multi_turn", gathered)

    assert reduced == {"multi_turn/multi_turn_metric/round_number_mean": pytest.approx(3.0)}
...
def test_log_multi_turn_data_wires_extrema_reductions(monkeypatch):
    captured = {}
    ...
    assert captured["reduction_by_key"] == {
        key: "max" if key.endswith("_max") else "min" for key in captured["log_dict"] if key.endswith(("_max", "_min"))
    }
